...
else 
{
    ...
    #From Github
    & wsl -d LocalDockerHost -e sh -c "echo 'Attempting to download installation script...' && echo 'nameserver 8.8.8.8' >> /etc/resolv.conf && wget -q https://raw.githubusercontent.com/Shurugwi/Wsl2Docker/main/InstallPrereqs.sh -O - | bash && exit"
    
    #Local Dev
    # & wsl -d LocalDockerHost -e sh -c "echo 'Attempting to download installation script...' && cp /mnt/d/Projects2021/Wsl2Docker/InstallPrereqs.sh ~/id.sh && tr -d '\15\32' < ~/id.sh > ~/id2.sh && chmod +x ~/id2.sh && bash ~/id2.sh && exit"

    & wsl -t LocalDockerHost
}
